...
while True:
    ...
    while True:
        ...
        _temp = df[df['candle_begin_time_GMT8'] == (
            run_time - timedelta(minutes=int(time_interval.strip('m'))))]
        if _temp.empty:
    ...
    if trade_coin_amount > 0 and signal == 0:
        ...
        # 下单
        place_order(exchange, order_type='limit', buy_or_sell='sell',
                    symbol=symbol, price=price*0.98, amount=trade_coin_amount)
        # 邮件标题
    ...
    if trade_coin_amount == 0 and signal == 1:
        ...
        # 获取最新的卖出价格
        place_order(exchange, order_type='limit', buy_or_sell='buy',
                    symbol=symbol, price=price*1.02, amount=buy_amount)
        # 邮件标题
